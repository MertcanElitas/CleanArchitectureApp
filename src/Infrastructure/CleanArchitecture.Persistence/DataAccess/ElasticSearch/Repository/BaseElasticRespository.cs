...
{
    public abstract class BaseElasticRespository<T> : IBaseElasticRepository<T> where T : BaseElasticEntity
    {
        private readonly IElasticClient _elasticClient;

        public BaseElasticRespository (IElasticProvider elasticProvider)
        {
        ...
        {
            var result =
                await _elasticClient.SearchAsync<T>(s => s.Query(p => NestExtensions.BuildMultiMatchQuery<T>(term)));
        ...
        {
            var response =
                await _elasticClient.UpdateAsync(DocumentPath<T>.Id(model.Id).Index(IndexName), p => p.Doc(model));
